    ...
    def main(self, *, args):
        with NodeStrategy(args) as node:
            ...
            if args.mocap_system in mocap_systems:
                ...
                if ok:
                    if lc_status.id == LCState.PRIMARY_STATE_ACTIVE:
                        node.get_logger().warn(args.mocap_system + ' already started')
                    elif lc_status.id != LCState.PRIMARY_STATE_INACTIVE:
                        node.get_logger().warn('Unable to start ' + args.mocap_system)
                    else:
                        start_ok = send_start_control(node, args.mocap_system)
                        lc_status, ok = get_lc_status(node, args.mocap_system)

                        if not start_ok or not ok or lc_status.id != LCState.PRIMARY_STATE_ACTIVE:
                            node.get_logger().error('Error starting ' + args.mocap_system)
                        else:
